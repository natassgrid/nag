...
export interface PaperGenerateDialogData {
  examId?: string;
}
...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>Generate Question Paper</h2>

    <mat-dialog-content>
      <mat-tab-group [(selectedIndex)]="activeTab" animationDuration="150ms">

        <!-- ══════════════════════════════════════════════════════════════
             TAB 1 — Build blueprint
        ══════════════════════════════════════════════════════════════════ -->
...
export class PaperGenerateDialogComponent implements OnInit {

  form!: FormGroup;
  templateNameCtrl = this.fb.control('');
  templateDescCtrl = this.fb.control('');
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<PaperGenerateDialogComponent>,
    private paperService: PaperService,
    private snackBar: MatSnackBar,
    @Inject(MAT_DIALOG_DATA) public data: PaperGenerateDialogData
  ) {}
  ...
  loadTemplates(): void {
    ...
    const examId = this.form.get('examId')?.value?.trim();
    this.paperService
      .getTemplates(examId && this.UUID_PATTERN.test(examId) ? examId : undefined)
      .pipe(
        catchError(() => of([])),
  ...
  saveCurrentAsTemplate(): void {
    ...
    const request = {
      ...
      examId: examId && this.UUID_PATTERN.test(examId) ? examId : undefined,
      rules: this.rulesValue()
    };

    this.savingTemplate = true;

    // If the currently loaded template has the same name, update it; otherwise create.
