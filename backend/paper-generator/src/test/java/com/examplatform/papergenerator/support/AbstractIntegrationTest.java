...
package com.examplatform.papergenerator.support;
...
/**
 * Production-grade base class for paper-generator MockMvc integration tests using Testcontainers.
 */
...
public abstract class AbstractIntegrationTest {
    ...
    static {
        try {
            POSTGRES_CONTAINER = new PostgreSQLContainer<>(POSTGRES_IMAGE)
                    .withDatabaseName("test_paper_generator_db")
    ...
    static void configureContainerProperties(DynamicPropertyRegistry registry) {
        if (testcontainersAvailable && POSTGRES_CONTAINER != null && POSTGRES_CONTAINER.isRunning()) {
            ...
            registry.add("spring.flyway.schemas", () -> "paper_generator");
            registry.add("spring.flyway.default-schema", () -> "paper_generator");
            ...
            try {
                List<String> tableNames = jdbcTemplate.queryForList(
                        "SELECT table_name FROM information_schema.tables " +
                                "WHERE table_schema = 'paper_generator' " +
                                "AND table_type = 'BASE TABLE' " +
                                "AND table_name NOT LIKE 'flyway_schema_history%' AND table_name != 'blueprint_template'",
                        String.class
                );

                for (String table : tableNames) {
                    jdbcTemplate.execute("TRUNCATE TABLE paper_generator." + table + " RESTART IDENTITY CASCADE");
                }
            } catch (Exception ex) {
