...
/**
 * Broadcast RabbitMQ Listener that subscribes to 'system.config.events'.
 * Creates an anonymous, exclusive, auto-delete queue bound to 'exam.events' topic exchange
 * to receive real-time invalidation notifications and update its L1 Near Cache.
 */
@Slf4j
@RequiredArgsConstructor
public class DynamicConfigRabbitInvalidationListener {
    ...
    @RabbitListener(
            bindings = @QueueBinding(
                    value = @Queue(value = "#{T(java.util.UUID).randomUUID().toString()}", autoDelete = "true", exclusive = "true"),
                    exchange = @Exchange(value = "exam.events", type = ExchangeTypes.TOPIC),
                    key = CONFIG_EVENTS_TOPIC
            )
    )
    public void onConfigChangeEvent(Object message) {
        try {
            ...
            if (event != null && event.paramName() != null) {
                ...
                log.info("L1 Near Cache updated via RabbitMQ invalidation for param '{}' (tenant: {}) to '{}'",
                        event.paramName(), event.tenantId(), event.newValue());
            }
        } catch (Exception e) {
            log.warn("Failed to process system configuration invalidation event via RabbitMQ: {}", e.getMessage());
        }
    }
