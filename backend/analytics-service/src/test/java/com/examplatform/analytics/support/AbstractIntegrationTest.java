...
package com.examplatform.analytics.support;
...
/**
 * Production-grade base class for analytics-service MockMvc integration tests using Testcontainers.
 */
...
public abstract class AbstractIntegrationTest {
    ...
    static {
        try {
            POSTGRES_CONTAINER = new PostgreSQLContainer<>(POSTGRES_IMAGE)
                    .withDatabaseName("test_analytics_db")
    ...
    static void configureContainerProperties(DynamicPropertyRegistry registry) {
        if (testcontainersAvailable && POSTGRES_CONTAINER != null && POSTGRES_CONTAINER.isRunning()) {
            ...
            registry.add("spring.flyway.schemas", () -> "analytics_service");
            registry.add("spring.flyway.default-schema", () -> "analytics_service");
            ...
            try {
                List<String> tableNames = jdbcTemplate.queryForList(
                        "SELECT table_name FROM information_schema.tables " +
                                "WHERE table_schema = 'analytics_service' " +
                ...
                for (String table : tableNames) {
                    jdbcTemplate.execute("TRUNCATE TABLE analytics_service." + table + " RESTART IDENTITY CASCADE");
                }
            } catch (Exception ex) {
