...
-- EXAM_CONTROLLER: Exam management + Live delivery monitor/proctor/retest
INSERT INTO identity_service.role_permission (id, tenant_id, role_id, permission_id)
SELECT
    gen_random_uuid(),
    'default',
    ...
    p.id
FROM identity_service.role_definition r
JOIN identity_service.permission p ON p.module IN ('EXAM_MANAGEMENT', 'EXAM_DELIVERY')
...
SELECT
    ...
    p.id
FROM identity_service.role_definition r
JOIN identity_service.permission p ON p.module = 'ASSESSMENT_EVALUATION'
WHERE r.code = 'EVALUATOR'
ON CONFLICT (role_id, permission_id, tenant_id) DO NOTHING;

-- AUDITOR: Audit & Analytics reading
INSERT INTO identity_service.role_permission (id, tenant_id, role_id, permission_id)
SELECT
    gen_random_uuid(),
    'default',
    ...
    p.id
FROM identity_service.role_definition r
JOIN identity_service.permission p ON p.code IN ('AUDIT:READ', 'AUDIT:EXPORT', 'ANALYTICS:VIEW', 'REPORTS:EXPORT')
WHERE r.code = 'AUDITOR'
ON CONFLICT (role_id, permission_id, tenant_id) DO NOTHING;

-- CANDIDATE: Attend exam, minimal view
INSERT INTO identity_service.role_permission (id, tenant_id, role_id, permission_id)
SELECT
    gen_random_uuid(),
    'default',
    ...
    p.id
FROM identity_service.role_definition r
JOIN identity_service.permission p ON p.code = 'DELIVERY:ATTEND'
