...
package com.examplatform.audit.support;
...
/**
 * Production-grade base class for audit-service MockMvc integration tests using Testcontainers.
 */
...
public abstract class AbstractIntegrationTest {
    ...
    static void configureContainerProperties(DynamicPropertyRegistry registry) {
        if (testcontainersAvailable && POSTGRES_CONTAINER != null && POSTGRES_CONTAINER.isRunning()) {
            ...
            registry.add("spring.flyway.schemas", () -> "audit_service");
            registry.add("spring.flyway.default-schema", () -> "audit_service");
            ...
            try {
                List<String> tableNames = jdbcTemplate.queryForList(
                        "SELECT table_name FROM information_schema.tables " +
                                "WHERE table_schema = 'audit_service' " +
                ...
                for (String table : tableNames) {
                    jdbcTemplate.execute("TRUNCATE TABLE audit_service." + table + " RESTART IDENTITY CASCADE");
                }
            } catch (Exception ex) {
