...
package com.examplatform.response.support;
...
/**
 * Production-grade base class for response-service MockMvc integration tests using Testcontainers.
 */
...
public abstract class AbstractIntegrationTest {
    ...
    static {
        try {
            POSTGRES_CONTAINER = new PostgreSQLContainer<>(POSTGRES_IMAGE)
                    .withDatabaseName("test_response_db")
    ...
    static void configureContainerProperties(DynamicPropertyRegistry registry) {
        if (testcontainersAvailable && POSTGRES_CONTAINER != null && POSTGRES_CONTAINER.isRunning()) {
            ...
            registry.add("spring.flyway.schemas", () -> "response_service");
            registry.add("spring.flyway.default-schema", () -> "response_service");
            ...
            try {
                List<String> tableNames = jdbcTemplate.queryForList(
                        "SELECT table_name FROM information_schema.tables " +
                                "WHERE table_schema = 'response_service' " +
                ...
                for (String table : tableNames) {
                    jdbcTemplate.execute("TRUNCATE TABLE response_service." + table + " RESTART IDENTITY CASCADE");
                }
            } catch (Exception ex) {
