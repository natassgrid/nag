...
package com.examplatform.questionbank.support;
...
/**
 * Production-grade base class for question-bank-service MockMvc integration tests using Testcontainers.
 */
...
public abstract class AbstractIntegrationTest {
    ...
    static {
        try {
            POSTGRES_CONTAINER = new PostgreSQLContainer<>(PGVECTOR_IMAGE)
                    .withDatabaseName("test_question_db")
    ...
    static void configureContainerProperties(DynamicPropertyRegistry registry) {
        if (testcontainersAvailable && POSTGRES_CONTAINER != null && POSTGRES_CONTAINER.isRunning()) {
            ...
            registry.add("spring.flyway.schemas", () -> "question_service");
            registry.add("spring.flyway.default-schema", () -> "question_service");
            ...
            try {
                List<String> tableNames = jdbcTemplate.queryForList(
                        "SELECT table_name FROM information_schema.tables " +
                                "WHERE table_schema = 'question_service' " +
                ...
                for (String table : tableNames) {
                    jdbcTemplate.execute("TRUNCATE TABLE question_service." + table + " RESTART IDENTITY CASCADE");
                }
            } catch (Exception ex) {
