...
-- Table: translation
-- ============================================================
-- Stores localized versions of questions in all 22 Eighth Schedule
-- official Indian languages (ISO 639 codes: hi, bn, te, mr, ta,
-- ur, gu, kn, ml, or, pa, as, mai, sa, sd, ne, kok, doi, mni,
-- sat, bo, kas).
--
-- translated_payload  — JSON blob: { content, options:[{id,text}], explanation }
--                       Stored as plain text by default.
--                       Stored as Vault ciphertext when
--                       app.translation.encryption.enabled=true
--                       (per-row flag: payload_encrypted).
--
-- source_version      — question.version captured at translation time;
--                       used to detect staleness without re-querying question.
--
-- IDs are UUID v7, generated by application (UuidV7Generator).
-- ============================================================
CREATE TABLE IF NOT EXISTS question_service.translation (
    ...
    updated_at          TIMESTAMP NOT NULL DEFAULT NOW(),
    version             BIGINT NOT NULL DEFAULT 0,
    CONSTRAINT uq_translation_question_lang_tenant
        UNIQUE (question_id, language_code, tenant_id)
);

CREATE INDEX IF NOT EXISTS idx_translation_tenant_id
...
CREATE INDEX IF NOT EXISTS idx_translation_status
    ON question_service.translation (status);

-- ============================================================
-- Table: batch_translation_job
...
CREATE INDEX IF NOT EXISTS idx_batch_trans_job_paper_id
    ON question_service.batch_translation_job (paper_id) WHERE paper_id IS NOT NULL;
...
FROM (VALUES
    ...
    -- General Hindi
    ('default', 'General Hindi', 'वर्ण विचार एवं वर्तनी शुद्धिकरण', 'स्वर, व्यंजन, उच्चारण स्थान, और वर्तनी की अशुद्धियों का संशोधन'),
    ...
    ('default', 'General Hindi', 'मुहावरे एवं लोकोक्तियां', 'प्रचलित मुहावरों, कहावतों के अर्थ और संदर्भानुसार प्रयोग'),
...
FROM (VALUES
    ...
    ('default', 'General Hindi', 'वाक्य शुद्धि एवं त्रुटि पहचान', 'कारक एवं वचन संबंधी अशुद्धियां'),
