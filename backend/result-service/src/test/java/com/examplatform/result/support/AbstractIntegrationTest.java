...
package com.examplatform.result.support;
...
/**
 * Production-grade base class for result-service MockMvc integration tests using Testcontainers.
 */
...
public abstract class AbstractIntegrationTest {
    ...
    static {
        try {
            POSTGRES_CONTAINER = new PostgreSQLContainer<>(POSTGRES_IMAGE)
                    .withDatabaseName("test_result_db")
    ...
    static void configureContainerProperties(DynamicPropertyRegistry registry) {
        if (testcontainersAvailable && POSTGRES_CONTAINER != null && POSTGRES_CONTAINER.isRunning()) {
            ...
            registry.add("spring.flyway.schemas", () -> "result_service");
            registry.add("spring.flyway.default-schema", () -> "result_service");
            ...
            try {
                List<String> tableNames = jdbcTemplate.queryForList(
                        "SELECT table_name FROM information_schema.tables " +
                                "WHERE table_schema = 'result_service' " +
                ...
                for (String table : tableNames) {
                    jdbcTemplate.execute("TRUNCATE TABLE result_service." + table + " RESTART IDENTITY CASCADE");
                }
            } catch (Exception ex) {
