...
package com.examplatform.delivery.support;
...
/**
 * Production-grade base class for delivery-service MockMvc integration tests using Testcontainers.
 */
...
public abstract class AbstractIntegrationTest {
    ...
    static {
        try {
            POSTGRES_CONTAINER = new PostgreSQLContainer<>(POSTGRES_IMAGE)
                    .withDatabaseName("test_delivery_db")
    ...
    static void configureContainerProperties(DynamicPropertyRegistry registry) {
        if (testcontainersAvailable && POSTGRES_CONTAINER != null && POSTGRES_CONTAINER.isRunning()) {
            ...
            registry.add("spring.flyway.schemas", () -> "delivery_service");
            registry.add("spring.flyway.default-schema", () -> "delivery_service");
        ...
        if (testcontainersAvailable && jdbcTemplate != null) {
            try {
                ...
                for (String table : tableNames) {
                    jdbcTemplate.execute("TRUNCATE TABLE delivery_service." + table + " RESTART IDENTITY CASCADE");
                }
            } catch (Exception ex) {
